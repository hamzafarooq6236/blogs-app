...
export default function SignUp() {
    ...
    async function submit(data: FormInputs) {
        ...
        try {
            ...
            if (res.success) {
                setMessage(res.message || "Signup successful! You can now sign in.");
            } else {
            ...
            <div className="max-w-md w-full z-10">
                ...
                <div className="bg-white/60 border border-[#CBF1F5] rounded-[2rem] p-8 sm:p-10 shadow-xl shadow-[#A6E3E9]/20 backdrop-blur-xl">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-slate-500 mt-2 text-sm">Join us to start writing and sharing.</p>
                    </div>

                    <form onSubmit={handleSubmit(submit)} className="flex flex-col gap-4">
                        {message && (
                            <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl text-emerald-600 text-sm font-medium text-center">
                                {message}
                            </div>
                        )}
                        {errorMsg && (
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            />
                            {errors.name?.message && (<p className="text-red-500 text-xs ml-1">{errors.name?.message}</p>)}
                        </div>
                        ...
                        <button 
                            type="submit" 
                            disabled={isLoading}
                            className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#71C9CE] hover:bg-[#5bb8bd] disabled:bg-[#A6E3E9] text-white rounded-xl shadow-md shadow-[#71C9CE]/20 transition-all font-semibold mt-3"
                        >
                    ...
                    <p className="text-center text-sm text-slate-500 mt-8">
                        ...
                        <Link href="/auth/signin" className="text-[#71C9CE] hover:text-[#5bb8bd] font-semibold transition-colors">
                            Sign in
                        </Link>
                    </p>
                </div>
